	...
	public function run(array $input, string $mode = 'confirm'): array
	{
		...
		try {
			...
			if ($existing instanceof WC_Order) {
				$items = array_values($existing->get_items('line_item'));
				if (isset($items[0])) {
					$quantity = (int) $items[0]->get_quantity();
					$data['product_id'] = (int) $items[0]->get_product_id();
				}
				$need_stock_headroom = !$this->stock->is_reduced($existing);
			}

			$product = $this->adapter->validate_ticket_product((int) $data['product_id'], $quantity, $need_stock_headroom);
			if (!$product['ok'] || !$product['product'] instanceof WC_Product) {
				return $this->fail_result($product['errors'], $existing, $import_id);
			}

			if ($mode === 'retry_email' && $existing) {
			...
			$stock_ok = $this->ensure_stock($order, $product['product'], $quantity);
			if (!$stock_ok['ok']) {
				return $stock_ok;
			}
